...
{
	const struct v4l2_mbus_framefmt *img_fmt, *ed_fmt;
	struct v4l2_subdev_state *state;
	u32 img_code, ed_code;

	if (pad != IMX219_PAD_SOURCE)
			return -EINVAL;

	state = v4l2_subdev_lock_and_get_active_state(sd);
	img_fmt = v4l2_subdev_state_get_format(state, IMX219_PAD_SOURCE,
										IMX219_STREAM_IMAGE);
	img_code = img_fmt->code;
	ed_fmt = v4l2_subdev_state_get_format(state, IMX219_PAD_SOURCE,
										IMX219_STREAM_EDATA);
	ed_code = ed_fmt->code;
	...
	fd->entry[0].bus.csi2.dt = imx219_get_format_bpp(img_fmt) == 8 ?
...
static u32 imx219_format_edata(const struct v4l2_mbus_framefmt *format)
{
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	*format = fmt->format;
	...
	ed_format->code = imx219_format_edata(format);
