...
raw formats. The :ref:`V4L2_CID_COLOR_PATTERN
<image-source-control-color-pattern>` control in the same sub-device defines the
native color pattern of the device. Flipping may further affect the readout
pattern as indicated by the :ref:`V4L2_CID_COLOR_PATTERN_FLIP
<image-source-control-color-pattern-flip>` control. Further on, cropping also
has an effect on the pattern if cropped amount is not divisible by the size of
the pattern, horizontally and vertically.
